...
public class adminMemberService {
	...
	// 判斷登入的帳號密碼
	/*
	 * public ModelAndView getByAccountAndPassword(HttpSession session,String
	 * account,String password) { adminMember
	 * member=dao.findByAccountAndPassword(account, password); if(member!=null) {
	 * ModelAndView model=new ModelAndView("adminAccount");
	 * model.addObject("adminmember", member); session.setAttribute("adminMember",
	 * member); return model; } ModelAndView model=new ModelAndView("adminLogin");
	 * model.addObject("adminmember", "查無此管理員"); return model; }
	 */
